...
%   does not error if it tries to look up that field.
    ...
    bounds.bevelRingWeight = [0.0, 1.5];         % reinforcement at inclined (bevel) joint face
    bounds.bevelRingWidthFactor = [0.5, 12.0];  % bevel-ring Gaussian width / nominal element size
    bounds.flatRingWeight = [0.0, 1.5];          % reinforcement at perpendicular (flat) face
    bounds.flatRingWidthFactor = [0.5, 12.0];   % flat-ring Gaussian width / nominal element size
