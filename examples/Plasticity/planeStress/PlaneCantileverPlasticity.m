...
mesh = Mesh();
mesh.addRectMesh2D(0, 0, 4*l, l, 4*res, res, sfL4.pattern);
fixedEdgeSelector = Selector( @(x)( abs(x(:,1)))<0.001 );
loadedEdgeSelector = Selector( @(x)( abs(x(:,1) - 4*l)<0.001 ) );
...
analysis.elementLoadLineIntegral( "global",loadedEdgeSelector, ["ux" "uy"], @(x)( x*0 + [0 -10] ));
