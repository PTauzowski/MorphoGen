classdef StressIntensityMultiTopologyOptimization < StressIntensityTopologyOptimizationVol
    ...
    properties
        ...
        alphas;              % per-load-case weights -- D3
    end

    methods (Static)
        function [alphas, maxais, penal, volFr, is_const] = parseWeightedArgs(nLoadCases, varargin)
            % Two call forms are in use and both are supported -- D3.
            %   (maxais, penal, volFr, is_const)          CAS_Arm, unweighted
            %   (alphas, maxais, penal, volFr, is_const)  Vibrations, weighted
            % The weighted form is what makes the vibration study's alpha sweep
            % mean anything; defaulting to uniform keeps the six-argument sites
            % working untouched.
            if numel(varargin) == 5
                [alphas, maxais, penal, volFr, is_const] = varargin{:};
            elseif numel(varargin) == 4
                [maxais, penal, volFr, is_const] = varargin{:};
                alphas = ones(1, nLoadCases);
            else
                error('StressIntensityMultiTopologyOptimization:Arity', ...
                    'Expected 4 or 5 trailing arguments, got %d.', numel(varargin));
            end
        end
    end
    ...
    methods
        function obj = StressIntensityMultiTopologyOptimization( ...
                Rmin, FEAnalyses, alphas, maxais, penal, volFr, is_const, intensityAggregation)
            ...
            if isempty(alphas)
                alphas = ones(1, numel(FEAnalyses));
            end
            assert(numel(alphas) == numel(FEAnalyses), ...
                'StressIntensityMultiTopologyOptimization:AlphasArity', ...
                'alphas has %d entries for %d load cases.', numel(alphas), numel(FEAnalyses));
            obj.alphas = alphas;
        end
        ...
        function ais = computeAverageIntensities(obj)
            ...
            alphas = obj.alphas;      % local copy: parfor cannot slice obj
